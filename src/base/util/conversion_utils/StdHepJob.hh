...
////
//
// The basic StdHepJob track. Note that access controls are
...
////
//
// The StdHepJob class is the "handle" for the StdHep file, and
// provides the access mechanism to the events.
//
////
class StdHepJob : public lXDR {
private:
//
...
public:
   ...
   static const char *getText(void)  {
      static char buff[80];
      sprintf(buff, "StdHepJob version %d.%d (%02d.%02d.%d) by W.G.J. Langeveld, SLAC",
...
//
   StdHepJob(const char *filename = 0, bool open_for_write = false);
//
// Prevent copying:
//
private:
   StdHepJob(const StdHepJob &);
public:
   virtual ~StdHepJob();
//
// Prevent assignment:
//
private:
   StdHepJob       &operator=(const StdHepJob &);
public:
//
