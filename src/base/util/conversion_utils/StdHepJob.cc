...
//
StdHepJob::StdHepJob(const char *filename, bool open_for_write) : 
...
void StdHepJob::printFileHeader(FILE *fp)
{
   if (fp == 0) fp = stdout;
...
void StdHepJob::printEventTable(FILE *fp)
{
   if (fp == 0) fp = stdout;
   eventTable.print(fp);
   return;
...
void StdHepJob::printEventHeader(FILE *fp)
{
   if (fp == 0) fp = stdout;
...
void StdHepJob::printEvent(FILE *fp)
{
   if (fp == 0) fp = stdout;
   event.print(fp);
   return;
...
void StdHepJob::printTrack(int i, FILE *fp)
{
   if (fp == 0) fp = stdout;
   if (i < event.nhep) {
...
void StdHepJob::printBeginRunRecord(FILE *fp)
{
   if (fp == 0) fp = stdout;
   return;
}

void StdHepJob::printEndRunRecord(FILE *fp)
{
   if (fp == 0) fp = stdout;
   return;
}

bool StdHepJob::more(void)
{
   return(getError() == LSH_SUCCESS);
}

long StdHepJob::readEvent(void)
{
//
...
long StdHepJob::getEvent(lStdEvent &lse) const
{
...
long StdHepJob::readEvent(lStdEvent &lse)
{
...
long StdHepJob::readFileHeader(void)
{
   long len, blockid;
...
long StdHepJob::writeEvent(void)
{
   return(LSH_NOTSUPPORTED);
}

long StdHepJob::setEvent(const lStdEvent &lse)
{
// ***************set up event buffer!
   setNTracks(lse.size());
...
long StdHepJob::writeEvent(lStdEvent &lse)
{
...
StdHepJob::EventTable::EventTable() :
...
void StdHepJob::EventTable::cleanup(void)
{
...
long StdHepJob::EventTable::read(StdHepJob &ls)
{
   long len;
...
long StdHepJob::EventTable::print(FILE *fp)
{
...
void StdHepJob::Event::cleanup(void)
{
   delete [] version;     version   = 0;
...
long StdHepJob::Event::read(StdHepJob &ls)
{
//
// Read event header
//
...
long StdHepJob::Event::printHeader(FILE *fp)
{
...
long StdHepJob::Event::print(FILE *fp)
{
