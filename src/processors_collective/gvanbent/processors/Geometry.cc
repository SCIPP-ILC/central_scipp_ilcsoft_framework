...
void Geometry::init() { 
    ...
    _hitmap = new TH2F("hitmap","Hit Distribution",300.0,-150.0,150.0,300.0,-150.0,150.0);  //creates scatterplot with name "hitmap", #of bins, xmin, xmax, # of y bins, ymin, ymax
...
void Geometry::processEvent( LCEvent * evt ) { 
    ...
    if( col != NULL ){
	   ...
	   MCParticle* particle = dynamic_cast<MCParticle*>( col->getElementAt(hitIndex) ); //col->getElementAt() same as  (*col).getElementAt()
