...
measure Will::getMeasure(LCCollection* col){
  ...
  vector<fourvec> hadronic_system;
  ...
  for(auto particle: all_particles){
	...
	out.electronic=out.positron;
      ...
      hadronic_system.push_back(hadron);
      out.hadronic+=hadron;
  ...
  if(out.scattered){
    ...
    out.hadronic=*(new fourvec);
    for(fourvec hadron: hadronic_system){
      const double angle = 0.5; //Cutting angle
      if(abs(hadron.z/getMag(hadron)) < angle) out.hadronic+=hadron;
    }
