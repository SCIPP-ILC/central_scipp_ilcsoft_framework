...
static TH1F* _MRcm_T;
static TH1F* _MRcm_DAB;
static TH1F* _MRcm_DED; 

static TH1F* _MR0_T;
static TH1F* _MR0_DAB;
static TH1F* _MR0_DED; 

static TH1F* _MR1_T;
static TH1F* _MR1_DAB;
static TH1F* _MR1_DED; 

SusyRazor SusyRazor;

SusyRazor::SusyRazor() : Processor("SusyRazor") {
...
void SusyRazor::processRunHeader( LCRunHeader* run) { 
    //run->parameters().setValue("thrust",12300321);
    //    _nRun++ ;
} 

void SusyRazor::processEvent( LCEvent * evt ) { 
    ...
    double j14vecR[4] = {gamma*vec[0][d][0]-gamma*beta*vec[0][d][3], vec[0][d][1], vec[0][d][2], 
        -gamma*beta*vec[0][d][0]+gamma*vec[0][d][3]};
    double j24vecR[4] =  {gamma*vec[1][d][0]-gamma*beta*vec[1][d][3], vec[1][d][1], vec[1][d][2], 
        -gamma*beta*vec[1][d][0]+gamma*vec[1][d][3]};
    ...
    if(beta2<=1){
        ...
        if(d==1){
            _R_DAB->Fill(R);
            _MR0_DAB->Fill(2*pj1R);
            _MR1_DAB->Fill(2*pj2R);
        }
        if(d==2){
            _R_DED->Fill(R);
            _MR0_DED->Fill(2*pj1R);
            _MR1_DED->Fill(2*pj2R);
        }
        if(partMom1){
...
void SusyRazor::check( LCEvent * evt ) { 
...
int SusyRazor::TassoThrustRazor(){
    int ThrustError = 0; 
    if(_partMom.size()<=0)
    {
        partMom0 = true; 
...
*/
int SusyRazor::JetsetThrustRazor(){
...
double SusyRazor::sign(double a, double b)
{
    if ( b < 0 )
    { return -fabs(a); } else { return fabs(a); }
}
//______________________________________________________________
double SusyRazor::min(double a, double b)
{
    if ( a < b )
